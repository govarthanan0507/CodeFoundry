...
When browser tooling is available, the worker must exercise the running application using Playwright CLI or the host's equivalent browser tool. The Playwright CLI reference explicitly supports agent-driven navigation, interaction, snapshots, screenshots, and browser testing. urlPlaywright CLI referencehttps://github.com/microsoft/playwright-cli
