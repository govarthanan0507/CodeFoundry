...
1. Infer the UX hierarchy.
2. Determine the appropriate visual direction.
3. Read existing `DESIGN.md` if present.
4. Create/update `DESIGN.md` when persistent design rules are needed.
5. Record material design decisions that affect implementation.
...
For UX-principle findings, use observation -> relevant principle -> user impact -> recommendation -> evidence. These heuristics inform critique but are not hard gates.
